...
### `rust_image` pin mismatch

`update --check` can also fail for a reason that has nothing to do with a generator upgrade. The
diff shows one line: the `rust_image` pin in `gen-circleci-orb.toml` no longer matches the one in
the generated `config.yml`. The pin is committed in both files, and it moved in only one.

Configure your pin management tool to update both copies in the same change. For Renovate that is
a custom manager for each — one matching the value in the toml, one matching the `rust_image:`
line in `.circleci/` — sharing a `groupName` so they land in a single pull request.
